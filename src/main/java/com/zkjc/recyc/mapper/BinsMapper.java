...
public interface BinsMapper {
    ...
    @Select("SELECT * FROM Bins WHERE employee_id =#{employeeId}")
    @Results({
            @Result(property = "employeeId",  column = "employee_id"),
            @Result(property = "binName", column = "bin_name"),
            @Result(property = "currentV", column = "current_v"),
    ...
    BinEntity getOne(String employeeId);
    ...
    @Update("UPDATE Bins SET bin_name=#{binName},current_v=#{currentV},max_v=#{maxV},area=#{area},temperature=#{temperature},boss_id=#{bossId} WHERE employee_id =#{employeeId}")
